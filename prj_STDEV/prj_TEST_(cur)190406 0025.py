...
def main():
    ...
    df = Preprocessing(df_orig)
    print('> Dataframe(preprocessed)\n{}\n'.format(df.head()))
    return()

    # (1-4) Data Analysis :: ㅇㅇㅇ
    df_smr = func_StdMeanRatio_01(df_neat)
    # print('> {}'.format(df_smr.head()))
    return()
...
def func_StdMeanRatio_01(df):
    # (21-1) 샘플로 첫 다섯자료의 표준편차를 평균으로 나누어 기준값으로 설정
    ...
    # (21-2) 시작점 고정하되 하나씩 추가 :: (아래는 전부 잘됨)
    df['val01'] = pd.Series(df['val0'][0:i].tolist() for i in range(1, df['val0'].size+1))
    df['val01'] = [df['val0'][0:i].tolist() for i in range(1, df['val0'].size+1)]
    df['val02'] = [np.std(i) for i in df['val01']]
    df['val03'] = [np.mean(i) for i in df['val01']]
    df['val04'] = df['val02'] / df['val03']
    print(df[['val01','val02','val03','val04']].head(5))

    # (21-3) 하나씩 추가하면서 시작점 변동 :: (아래는 전부 잘됨)
