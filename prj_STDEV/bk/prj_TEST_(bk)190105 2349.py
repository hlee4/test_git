...
def main():
    ...
    ax1.plot(ddf[10:20].apply(pd.Series))
    plt.show()
...
def Std_Mean_Ratio(df):
    ...
    # (2-00) experiment
    ref_sigma = pd.Series(df['val00'][0:5]).std() / pd.Series(df['val00'][0:5]).mean()
    print(ref_sigma)

    unit_sigma = pd.Series(
      pd.Series(df['val00'][0:i]).std() / pd.Series(df['val00'][0:i]).mean()
       for i in range(1,10))
    print(unit_sigma)
    # pf = df[5]
