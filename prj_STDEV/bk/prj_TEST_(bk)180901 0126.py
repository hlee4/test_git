...
def main():
    ...
    df['val5'] = pd.Series([df['val1'][0:i].tolist() for i in range(1,df['val2'].size)])
    # df['val6'] = pd.Series([stat.stdev(x) for x in df['val5'][2:].tolist()])
    print([stat.stdev(x) for x in df['val5'][2:].tolist()])
    # print(df.head(10))
    return()
    ...
    # lst_cum = [[*range(0,i)] for i in range(1,df['val2'].size)]  # (잘됨)
    lst_cum = [df['val1'][0:i].tolist() for i in range(1,df['val2'].size)]  # (잘됨)
    print(len(lst_cum))
    print(lst_cum)
    # result = [stat.stdev(x) for x in lst_cum[1:]]
    # print(len(result))
