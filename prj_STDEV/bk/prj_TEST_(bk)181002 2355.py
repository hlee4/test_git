...
def main():
    ...
    # (아래는 전부 잘됨)
    # df['val5'] = pd.Series(df['val1'][0:i].tolist()
    #                        for i in range(1, df['val1'].size+1))
    ...
    # print(df[['val14']].head(5))
    # return()

    # (아래는 전부 잘됨) :: 그런데 시간이 오래걸리네
    # lst1 = [[df['val1'][0+j:i].tolist()
    #         for i in range(1+j, df['val1'].size+1)]
    #         for j in range(df['val1'].size)]        # (잘됨)
    ...
    # df['val14'].plot(kind='line', ax=ax1)
    ax1.plot(df['val14'][2])
    ax1.plot(df['val14'][3])
    ax1.plot(df['val14'][4])
    ax1.plot(df['val14'][15])
    ax1.plot(df['val14'][6])
    # ax1.plot(lst4[20])
    # ax1.plot(lst4[10])
    # ax1.plot(lst4[5])
