...
def main():
    ...
    df['val9'] = pd.Series([df['val1'][0+j:i].tolist() for i in range(1+j,df['val1'].size)] for j in range(df['val1'].size))
    df['val10'] = [[df['val1'][0+j:i].tolist() for i in range(1+j,df['val1'].size)] for j in range(df['val1'].size)]
    print(df[['val9','val10']].head(50))
    # return()

    # fig, (ax1) = plt.subplots(nrows=1, ncols=1, figsize=(8,6))
    # df['val8'].plot(kind='line')
    # plt.show()

    # for i, p in enumerate(df['val5']):
    #     print(i, p[i-1:])
