...
def main():
    ...
    df['val9'] = pd.Series([df['val1'][0+j:i].tolist() for i in range(1+j,df['val1'].size)] for j in range(df['val1'].size))    #(잘됨)
    # df['val10'] = [[df['val1'][0+j:i].tolist() for i in range(1+j,df['val1'].size)] for j in range(df['val1'].size)]    # (이것도 잘됨)
    df['val11'] = pd.Series([np.std(x) for x in y] for y in df['val9'])   #(잘됨)
    df['val12'] = pd.Series([np.mean(x) for x in y] for y in df['val9'])  #(잘됨)
    # df['val13'] = df['val11'] / df['val12']
    # print(df['val13'])
    print(df[['val9','val11','val12']].head(5))

    # return()

    # fig, (ax1) = plt.subplots(nrows=1, ncols=1, figsize=(8,6))
    # df['val9'].plot(kind='line')
    # plt.show()
