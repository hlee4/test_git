...
def main():
    # (1-1) 지오코딩 :: (잘됨)
    files = glob.glob('./dn_files/01_입력정보*.xlsx')
    file = files[-1]
    print('> {}'.format(file))
    # df = pd.read_excel(file, sheet_name=0, header=0, usecols='A:D', nrows=10, encoding='cp949')
    df = pd.read_excel(file, sheet_name=0, header=1, usecols='A:Q', encoding='cp949')
    print('> {}\n'.format(df.head()))
    # return()
    ...
    for index, row in df_addrs.iterrows():
        if (row[13] == '(불명)'):
            coord = ConvertToGcode(index, row, row[12], 'ROAD')
            print('> {}\n'.format(coord))
            lst_coords.append(coord)
        else:
            coord = ConvertToGcode(index, row, row[13], 'PARCEL')
            print('> {}\n'.format(coord))
            lst_coords.append(coord)

    df_coords = pd.DataFrame(lst_coords, columns=['구분', '업소명', '주소타입', '주소', 'lon', 'lat'])
    print('> {}\n'.format(df_coords.head(20)))
    # df_coords.to_csv('./out_출력정보_cp949.csv', encoding='cp949')  # (encoding Err)
    # df_coords.to_csv('./out_출력정보_utf8.csv', encoding='utf-8')
    df_coords.to_excel('./02_출력정보.xlsx', index=False, encoding='cp949')
    return()
    ...
    #     ret_type = 'ROAD'  # [PARCEL(지번), ROAD(도로명), BOTH]
    #     addr = ConvertToRevGcode(index, row, ret_type)
    #     lst_addrs.append(addr)

    # df_addrs = pd.DataFrame(lst_addrs, columns=['구분', '업소명', '주소타입', '입력주소', 'lon', 'lat', '출력주소'])
    # # print('> {}\n'.format(df_addrs))
    # print('> {}\n'.format(df_addrs.head(20)))
    # # df_addrs.to_csv('./out_Addrs_cp949.csv', encoding='cp949')  # (encoding Err)
    # # df_addrs.to_csv('./out_Addrs_utf8.csv', encoding='utf-8')
    # df_addrs.to_excel('./out_Addrs.xlsx', index=False, encoding='cp949')
    # return()
...
def ConvertToRevGcode(index, row, ret_type):
    # (5-1) Set URL
    base = 'http://api.vworld.kr/req/address?service=address'
    opt1 = '&request=getAddress'
    opt2 = '&key=' + ath.api_key  # HJL's Only
    opt3 = '&format=json'         # [json, xml]
    opt4 = '&type=' + ret_type    # [PARCEL(지번주소), ROAD(도로명주소), BOTH]
    ...
    if (isOK == 'OK'):
        ...
        # print(f'> 주소: {addr}')
        print('> 구분: {}, 업소명: {}, 경도: {}, 위도: {}, 주소: {}'.format(
              row[0], row[1], row[2], row[3], row['lon'], row['lat'], addr))
        return([row[0], row[1], row[2], row[3], row['lon'], row['lat'], addr])
    elif (isOK == 'NOT_FOUND'):
        print('> === ERROR: Wrong Address!! ===')
        return([row[0], row[1], row[2], row[3], row['lon'], row['lat'], 0])
    else:
        print('> === ERROR: Unknown!! ===')
        return([row[0], row[1], row[2], row[3], row['lon'], row['lat'], -1])
