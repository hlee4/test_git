...
def main():
    ...
    for index, row in df_addrs.iterrows():
        if (row[2] == '(불명)'):
            coord = ConvertToGcode(row, row[3], 'ROAD')
            print('> {}\n'.format(coord))
            lst_coords.append(coord)
        else:
            coord = ConvertToGcode(row, row[2], 'PARCEL')
            print('> {}\n'.format(coord))
            lst_coords.append(coord)

    df_coords = pd.DataFrame(lst_coords, columns=['구분', '업소명', '주소', 'lon', 'lat'])
    print('> {}\n'.format(df_coords.head(20)))
    # df_coords.to_csv('./out_Coords_(지번주소)_cp949.csv', encoding='cp949')  # (Encoding Err)
    df_coords.to_csv('./out_Coords_(지번주소)_utf8.csv', encoding='utf-8')
    # df_coords.to_csv('./out_Coords_(도로명주소)_utf8.csv', encoding='utf-8')
    return()
    ...
    for index, row in df_coords.iterrows():
        addr = ConvertToRevGcode(row)
        lst_addrs.append(addr)

    df_addrs = pd.DataFrame(lst_addrs, columns=['구분', '업소명', 'lon', 'lat', '주소'])
    print('> {}\n'.format(df_addrs.head(20)))
    # df_addrs.to_csv('./out_Addrs_(지번주소)_cp949.csv', encoding='cp949')
    # df_addrs.to_csv('./out_Addrs_(지번주소)_utf8.csv', encoding='utf-8')
    # df_addrs.to_csv('./out_Addrs_(도로명주소)_utf8.csv', encoding='utf-8')
    return()
...
# (4) Geocoding :: (잘됨)
def ConvertToGcode(row, addr, type):
    # (4-1) Set URL
    ...
    opt4 = '&type=' + type        # [PARCEL(지번주소), ROAD(도로명주소)]
    opt5 = '&address=' + addr
    ...
    #     print('> 구분: {}, 업소명: {}, 주소: {}, 경도: {}, 위도: {}/n'.format(
    #           row[0], row[1], addr, lon, lat))
    #     return([row[0], row[1], addr, lon, lat])
    # elif (isOK == 'NOT_FOUND'):
    #     print('> === ERROR: Wrong Address!! ===')
    #     return([row[0], row[1], addr, 0, 0])
    # else:
    #     print('> === ERROR: Unknown!! ===')
    #     return([row[0], row[1], addr, -1, -1])
