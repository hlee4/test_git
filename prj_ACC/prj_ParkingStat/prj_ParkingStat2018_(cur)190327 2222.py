...
def main():
    ...
    # df_occ = pd.read_excel(file_occ, sheet_name=0, header=0, usecols='A:O')
    ...
    # df_reg = pd.read_excel(file_reg, sheet_name=0, header=0, usecols='A:O')
    ...
    opt = 'ohlc'  # {'MMS': (max,mean,std), 'ohlc': (open,high,low,close)}
    ...
    # # resi_3d = residue_cal.xs(key=['지하체류량',Fri'], axis=0, level=[0,3])  # 샘플
    # resi_3d = residue_cal.xs(key=['지하체류량'], axis=0, level=[0])
    # resi_3d = resi_3d.xs(key=['max'], axis=1)
    # # print(f'> 체류대수(3D_max)\n{resi_3d.head(10)}\n')
    # fname_3d = './ACC_residue_3d_' + opt + '(' + cycle + ').csv'
    # resi_3d.to_csv(fname_3d, encoding='cp949')
...
def DataCleaning(opt):
    if opt == 'occasional':
        # (11-1) Combine several files into one file for OccasionalCustomer
        ...
        # (11-2) Combine several files into one file for RegularCustomer
...
def func_Duration_occ(df):
    ...
    for name in cats_names:
        ...
        # df_tmp = df[(df.index >= cats[name][0]) & (df.index < cats[name][1]) & (df['요금종별'] != '회차차량')]
...
def plot_Residue_ohlc(df):
    # print(df.index.levels[0])  # index-level0의 인덱스 값 반환
    df = df.xs(key=['지하체류량', 1], axis=0, level=[0, 1])
    # df = df.resample(rule='1D')
    print('> {}'.format(df.head()))
    # return()

    # # (54-2) mpl_finance 라이브러리를 이용한 플롯
    # # import matplotlib.pyplot as plt
    ...
    # # (거래량 차트)
    # # axes[0].bar(x, df.volume, color='k', width=0.6, align='center')
    ...
    #     _wd_prev = weekday
    # axes[1].set_ticks(_xticks)
    # axes[1].set_xticklabels(_xlabels, rotation=30, minor=False)
    ...
    # (54-3) mpl_finance 라이브러리를 이용한 플롯(다른버전)\
    print('> {}'.format(df.head()))
    # df_ohlc = df.xs(key=[1], axis=0, level=[0])
    df_ohlc = df.astype('int')
    df_ohlc.index = df_ohlc.index.droplevel(0)
    print('> {}'.format(df_ohlc))
    # print('> {}'.format(df_ohlc.dtypes)
    # return()
    ...
    # candlestick_ohlc(ax=ax1, quotes=zip(df.index.levels[0], df['open'],df['high'],df['low'],df['close']))  # 잘됨
    # candlestick_ohlc(ax=ax1, quotes=zip(df.index.levels[1], df['open'],df['high'],df['low'],df['close']))  # 에러
    # candlestick_ohlc(ax=ax1, quotes=zip(mdates.date2num(df_ohlc.index.astype(dt.date)), df_ohlc['open'],df_ohlc['high'],df_ohlc['low'],df_ohlc['close']))
    candlestick_ohlc(ax=ax1, quotes=zip(df_ohlc.index, df_ohlc['open'],df_ohlc['high'],df_ohlc['low'],df_ohlc['close']))
