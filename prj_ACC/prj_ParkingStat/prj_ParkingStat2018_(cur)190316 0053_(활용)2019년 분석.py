...
def main():
    ...
    files_occ = glob.glob('./dn_files/190325*일반*(190101~190228)*/*일반차량*.csv')
    ...
    files_reg = glob.glob('./dn_files/190325*정기*(190101~190228)*/*정기차량*.csv')
    ...
    resi_2d = resi_2d.xs(key=['max'], axis=1, level=[0])
    print(f'> 체류대수(2D)_종일\n{resi_2d.head(10)}\n')
    fname_2D = './ACC_residue_2D_' + opt + '(' + cycle + ').csv'
    resi_2d.to_csv(fname_2D, encoding='cp949')
...
def DataCleaning(opt):
    if opt == 'occasional':
        ...
        files = glob.glob('./dn_files/190325*일반*190101~190228*/월별파일/*.xlsx')
        df = pd.concat(pd.read_excel(file, sheet_name=0, skiprows=11, header=0,
        ...
        df.to_csv('2019년 일반차량 입출차기록.csv', encoding='cp949')
        print('> OK! Finished!')
    elif opt == 'regular':
        # (11-2) Combine several files into one file for RegularCustomer :: 잘됨
        files = glob.glob('./dn_files/190325*정기*190101~190228*/월별파일/*.xlsx')
        df = pd.concat(pd.read_excel(file, sheet_name=0, skiprows=13, header=0,
        ...
        df.to_csv('2019년 정기차량 입출차기록.csv', encoding='cp949')
        print('> OK! Finished!')
    else:
...
def func_Residue_occ(df, cycle):
    ...
    df_cum = df_cum[df_cum.index >= datetime.datetime(2019, 1, 1)]
    # print('> 시간대별 누적체류대수\n{}\n'.format(df_cum.tail(20)))
...
def plot_Residue_MMS_3D(df, opt1, opt2):    # .plot plots the index against every column
    ...
    if opt1 == '월별' and opt2 == 'plt':
        ...
        plt.suptitle('2019년도 외곽(지하)주차장 통계')
